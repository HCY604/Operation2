...
public class LoginSuccess extends JFrame {
    ...
    public LoginSuccess(Member m) {
        ...
        lbl.setBounds(30, 40, 320, 30);
        add(lbl);

        JButton btnGo = new JButton("前往購買電腦");
        btnGo.setBounds(30, 90, 140, 30);
        add(btnGo);

        JButton btnLogout = new JButton("登出");
        btnLogout.setBounds(190, 90, 140, 30);
        add(btnLogout);

        btnGo.addActionListener(e -> {
            new PurchaseLauncher(loginMember).open();
            dispose();
        });
